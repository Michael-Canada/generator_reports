...
MIN_MW_TO_BE_ANALYZED = 100  # PDF report threshold - generators included in reports if capacity OR generation >= this value
RUN_BID_VALIDATION = False

USE_THIS_MARKET = "ercot"  # Options: "miso", "spp", "ercot", "pjm"
MARKET_VERSION = (
    "ercot_env"  # it could be "ercot", "ercot_20250917", "ercot_20250911" etc.
)


# USE_THIS_MARKET = "miso"  # Options: "miso", "spp", "ercot", "pjm"
# MARKET_VERSION = "miso"  # it could be "ercot", "ercot_20250911" etc.
...
class Config:
    ...
    MARKET_VERSION = MARKET_VERSION  # example: "ercot", "ercot_env"
    ...
    BID_VALIDATION = {
        ...
        "gcs_config": {
            ...
            "base_paths": {
                ...
                "ercot": "metadata/ercot.resourcedb.v2/2025-08-29/",  # MS closess on 2025-09-19 for testing
                "ercot_env": "metadata/ercot.resourcedb.v2/2025-09-17/",
                "ercot_20250917": "metadata/ercot.resourcedb.v2/2025-09-17/",
    ...
    MARKET_CONFIGS = {
        ...
        "ercot": {  # MS closess on 2025-09-19 for testing
            # "run_version": "ercot_20250829",
            "run_version": "ercot_20250911",
            "collection": "ercot-rt-se",
            "timezone_offset": "T04:00:00-05:00",
        },
        "ercot_env": {
            "run_version": "ercot_20250917",
            "collection": "ercot-rt-se",
            "timezone_offset": "T04:00:00-05:00",
        },
        "ercot_20250917": {
    ...
    RESOURCEDB_INTEGRATION = {
        ...
        "file_paths": {
            ...
            "ercot": "metadata/ercot.resourcedb.v2/2025-08-29/resources.json",  # MS closess on 2025-09-19 for testing
            "ercot_env": "metadata/ercot.resourcedb.v2/2025-09-17/resources.json",
            "ercot_20250917": "metadata/ercot.resourcedb.v2/2025-09-17/resources.json",
    ...
    def get_current_config(cls):
        ...
        return cls.MARKET_CONFIGS[cls.MARKET_VERSION]
    ...
    def get_bid_validation_config(self):
        ...
        return {
            ...
            "reflow_collections": {
                "miso": "miso-se",
                "spp": "spp-se",
                "ercot": "ercot-rt-se",  # MS closess on 2025-09-19 for testing
                "ercot_env": "ercot-rt-se",
                "ercot_20250917": "ercot-rt-se",
                "pjm": "miso-se",
            },
        }
...
class DataProcessor:
    ...
    def extract_date_from_string(string: str, market: str) -> str:
        ...
        elif market in ["ercot", "ercot_env", "ercot_20250917"]:
    ...
    def convert_time(self, string: str) -> str:
        ...
        elif self.config.MARKET in ["ercot", "ercot_env", "ercot_20250917"]:
...
class GeneratorAnalyzer:
    ...
    def __init__(self, debug_mode=False):
        ...
        if self.config.MARKET in ["ercot", "ercot_env", "ercot_20250917"]:
    ...
    def _load_supply_curves_data(self):
        """Load supply curves data for comparison with ResourceDB."""
        try:
            from google.cloud import storage
            import json
            ...
            if not self.config.RESOURCEDB_INTEGRATION.get("enable_resourcedb", False):
                print(
                    "ResourceDB integration disabled - skipping supply curves comparison"
                )
                return {}

            bucket_name = self.config.RESOURCEDB_INTEGRATION["bucket_name"]

            # Construct supply curves path based on ResourceDB path
            resourcedb_path = self.config.RESOURCEDB_INTEGRATION["file_paths"][
                self.config.MARKET_VERSION
            ]
            # Replace resources.json with supply_curves.json
            supply_curves_path = resourcedb_path.replace(
                "resources.json", "supply_curves.json"
            )

            # Initialize the GCS client
            client = storage.Client()
            bucket = client.bucket(bucket_name)
            blob = bucket.blob(supply_curves_path)

            # Check if supply curves file exists
            if not blob.exists():
                print(f"Supply curves file not found: {supply_curves_path}")
                return {}

            # Download and parse supply curves data
            json_data = blob.download_as_text()
            supply_curves_list = json.loads(json_data)

            # Convert to dict for easier lookup (assuming similar structure to ResourceDB)
            if isinstance(supply_curves_list, list):
                supply_curves_db = {
                    item.get("uid", f"unknown_{i}"): item
                    for i, item in enumerate(supply_curves_list)
                }
            else:
                supply_curves_db = supply_curves_list

            print(f"Loaded supply curves data with {len(supply_curves_db)} entries")
            return supply_curves_db

        except ImportError:
            print(
                "Warning: google-cloud-storage not installed - supply curves comparison disabled"
            )
            return {}
        except Exception as e:
            print(f"Warning: Could not load supply curves data: {e}")
            return {}

    def _compare_resourcedb_vs_supply_curves(self):
        """Compare generators between ResourceDB and supply curves to find discrepancies."""
        if not hasattr(self, "resource_db") or not self.resource_db:
            print("ResourceDB not available - skipping comparison")
            return {}

        # Load supply curves data
        supply_curves_db = self._load_supply_curves_data()

        if not supply_curves_db:
            print("Supply curves data not available - skipping comparison")
            return {}

        # Store supply curves data for PDF table generation
        self.supply_curves_db = supply_curves_db

        # Filter out retired generators from both datasets
        print("🔍 Filtering out retired generators from comparison...")

        # Get all generator sets before filtering
        all_resourcedb_generators = set(self.resource_db.keys())
        all_supply_curves_generators = set(supply_curves_db.keys())

        # Filter out retired generators from ResourceDB
        active_resourcedb_generators = {
            gen_id
            for gen_id in all_resourcedb_generators
            if not self._is_generator_retired(gen_id)
        }

        # Filter out retired generators from Supply Curves
        # For supply curves, we need to check if they exist in ResourceDB and are retired there
        active_supply_curves_generators = {
            gen_id
            for gen_id in all_supply_curves_generators
            if not self._is_generator_retired(gen_id)
        }

        # Calculate retired counts
        retired_resourcedb_count = len(all_resourcedb_generators) - len(
            active_resourcedb_generators
        )
        retired_supply_curves_count = len(all_supply_curves_generators) - len(
            active_supply_curves_generators
        )

        print(f"   • Retired generators in ResourceDB: {retired_resourcedb_count}")
        print(
            f"   • Retired generators in Supply Curves: {retired_supply_curves_count}"
        )
        print(
            f"   • Active generators in ResourceDB: {len(active_resourcedb_generators)}"
        )
        print(
            f"   • Active generators in Supply Curves: {len(active_supply_curves_generators)}"
        )

        # Find active generators only in each dataset
        resourcedb_only = active_resourcedb_generators - active_supply_curves_generators
        supply_curves_only = (
            active_supply_curves_generators - active_resourcedb_generators
        )
        common_generators = (
            active_resourcedb_generators & active_supply_curves_generators
        )

        comparison_results = {
            "resourcedb_only": list(resourcedb_only),
            "supply_curves_only": list(supply_curves_only),
            "common_generators": list(common_generators),
            "total_resourcedb": len(active_resourcedb_generators),
            "total_supply_curves": len(active_supply_curves_generators),
            "resourcedb_only_count": len(resourcedb_only),
            "supply_curves_only_count": len(supply_curves_only),
            "common_count": len(common_generators),
            # Additional metadata about retired generators
            "retired_resourcedb_count": retired_resourcedb_count,
            "retired_supply_curves_count": retired_supply_curves_count,
            "total_resourcedb_including_retired": len(all_resourcedb_generators),
            "total_supply_curves_including_retired": len(all_supply_curves_generators),
        }

        print(f"\n📊 RESOURCEDB vs SUPPLY CURVES COMPARISON (ACTIVE GENERATORS ONLY):")
        print(
            f"  • Total active in ResourceDB: {comparison_results['total_resourcedb']}"
        )
        print(
            f"  • Total active in Supply Curves: {comparison_results['total_supply_curves']}"
        )
        print(f"  • Common active generators: {comparison_results['common_count']}")
        print(
            f"  • Only in ResourceDB (active): {comparison_results['resourcedb_only_count']}"
        )
        print(
            f"  • Only in Supply Curves (active): {comparison_results['supply_curves_only_count']}"
        )
        print(
            f"  📝 Note: Excluded {retired_resourcedb_count + retired_supply_curves_count} retired generators from comparison"
        )
    ...
    def _load_generators(self) -> pd.DataFrame:
        ...
        if self.config.GO_TO_GCLOUD:
            ...
            if generators is not None:
                generators.to_csv(
                    f"generators_from_reflow_{self.config.MARKET_VERSION}.csv",
                    index=False,
                )
        ...
        if self.config.GO_TO_GCLOUD:
            ...
            if self.names_all_generators is not None:
                self.names_all_generators.to_csv(
                    f"names_all_generators_{self.config.MARKET_VERSION}.csv",
                    index=False,
                )
        else:
    ...
    def _fetch_forecast_data(
        ...
        elif self.config.MARKET in ["ercot", "ercot_env", "ercot_20250917", "spp"]:
    ...
    def _merge_datasets(
        ...
        if len(merged_df) == 0 and self.config.MARKET in [
            "ercot",
            "ercot_env",
            "ercot_20250917",
        ]:
    ...
    def _generate_final_reports(
        ...
        try:
            ...
            # Pass data comparison results to the report generator if available
            if hasattr(self, "data_comparison_results"):
                report_generator.data_comparison_results = self.data_comparison_results
    ...
    def _display_final_summary(
        ...
        if len(ranked_results) > 0:
            ...
            is_ercot_status = (
                self.config.MARKET in ["ercot", "ercot_env", "ercot_20250917"]
    ...
    def run_batch_analysis(self):
        ...
        for batch_idx in range(num_batches):
            ...
            if self.config.SAVE_RESULTS and found_results:
                df_filename = f"generator__analyzer_{self.config.MARKET_VERSION}__{batch_idx}_{self.today_date_str}.csv"
                results_filename = f"generator_forecast_stats__{self.config.MARKET_VERSION}__{batch_idx}_{self.today_date_str}.csv"
        ...
        if all_results or self.config.MARKET in [
            "ercot",
            "ercot_env",
            "ercot_20250917",
        ]:
    ...
    def get_final_summary(self) -> dict:
        ...
        stats = {
            ...
            "analysis_date": self.today_date_str,
            "market": self.config.MARKET_VERSION.upper(),
        }
...
7. Strategic bidding improvements
    ...
    def save_csv_documentation(self) -> None:
        ...
        filename = (
            f"CSV_Documentation_{self.config.MARKET_VERSION}_{self.today_date_str}.txt"
        )
