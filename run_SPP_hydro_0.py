...
def _get_auth():
    return tuple(os.environ["MU_API_AUTH"].split(":"))  # MS ZZZ
...
if GO_TO_GCLOUD:
    ...
    generators.to_csv("generators_from_reflow.csv", index=False)
else:
    generators = pd.read_csv("generators_from_reflow.csv")
...
if GO_TO_GCLOUD:
    ...
    all_generators.to_csv("all_generators.csv", index=False)
    ...
    ng_hub_definition_parquet = pd.read_parquet("ng_hub_definition_parquet.parquet")
    all_generators = pd.read_csv("all_generators.csv")
...
if GO_TO_GCLOUD:
    ...
    names_all_generators.to_csv("names_all_generators.csv", index=False)
else:
    names_all_generators = pd.read_csv("names_all_generators.csv")
...
cntr_not_in_names_all_generators = 0
cntr_in_names_all_generators = 0
# cntr_not_in_MUSE = 0
...
for ii in range(len(generators)):
    ...
    #     pass
        ...
        df.to_csv(f"all_hydro_data{ii}.csv", index=False)
        df = pd.DataFrame(
            columns=[
                "case_converted",
                "name",
