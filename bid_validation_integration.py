...
def add_bid_validation_to_analyzer(analyzer_instance):
    ...
    analyzer_instance.bid_validator = BidValidator(
        ...
        config=analyzer_instance.config.get_bid_validation_config() if hasattr(analyzer_instance.config, 'get_bid_validation_config') else None
    )
    
    # Add validation results storage
    analyzer_instance.bid_validation_results = []
    ...
    analyzer_instance.get_bid_validation_summary = lambda: get_bid_validation_summary(analyzer_instance)
    analyzer_instance.save_bid_validation_results = lambda: save_bid_validation_results(analyzer_instance)
...
def run_bid_validation(analyzer_instance) -> pd.DataFrame:
    ...
    # Run validation
    validation_results = analyzer_instance.bid_validator.run_comprehensive_validation(generator_names)
    ...
    if not validation_results.empty:
        analyzer_instance.bid_validation_summary = {
            ...
            'critical_issues': len(validation_results[validation_results['severity'] == 'critical']),
            'high_issues': len(validation_results[validation_results['severity'] == 'high']),
            'medium_issues': len(validation_results[validation_results['severity'] == 'medium']),
            'generators_with_issues': validation_results['generator_name'].nunique(),
            'most_common_issue': validation_results['validation_type'].value_counts().index[0] if len(validation_results) > 0 else None,
...
def save_bid_validation_results(analyzer_instance) -> None:
    ...
    Args:
        analyzer_instance: GeneratorAnalyzer instance
    """
    if not hasattr(analyzer_instance, 'bid_validation_results') or analyzer_instance.bid_validation_results.empty:
        print("No bid validation results to save")
        return
    
    # Save detailed results
    timestamp = analyzer_instance.today_date_str
    detailed_filename = f"bid_validation_detailed_{analyzer_instance.config.MARKET}_{timestamp}.csv"
    ...
    summary_filename = f"bid_validation_summary_{analyzer_instance.config.MARKET}_{timestamp}.csv"
...
def enhance_final_reports_with_bid_validation(analyzer_instance, all_results: List[pd.DataFrame], 
                                           all_anomalies: List[pd.DataFrame], all_alerts: List[Dict]):
    """
    ...
    if not hasattr(analyzer_instance, 'bid_validation_results') or not analyzer_instance.bid_validation_results:
    ...
    if hasattr(analyzer_instance, 'bid_validation_summary'):
        ...
        if summary.get('total_issues', 0) > 0:
            ...
            print(f"Generators with bid issues: {summary.get('generators_with_issues', 0)}")
            ...
            if not analyzer_instance.bid_validation_results.empty:
                critical_high = analyzer_instance.bid_validation_results[
                    analyzer_instance.bid_validation_results['severity'].isin(['critical', 'high'])
                ]
                ...
                if not critical_high.empty:
                    ...
                    for _, row in top_issues.iterrows():
                        print(f"  {row['generator_name']}: {row['validation_type']} - {row['message'][:100]}...")
...
if __name__ == "__main__":
    ...
    bid_validation_config, get_config_method = create_enhanced_config_with_bid_validation()
