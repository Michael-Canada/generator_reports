...
def find_files(directory):
    ...
    for file in os.listdir(directory):
        if file.startswith('all_hydro_data'):
            files.append(file)
    return files
...
all_units_monthly_capacity_factor = pd.DataFrame(columns=['name', 'unit_id','month','capacity_factor'])
list_of_monthly_capacity_factor = []

all_units = df[['name', 'unit_id']].drop_duplicates().reset_index(drop=True)
for unit in all_units.values:
    unit_data = df.query('unit_id == @unit[1] and name == @unit[0]').sort_values('case_converted')
    aggregated_monthly = unit_data.groupby('month').agg({'capacity_factor':'mean'}).reset_index().sort_values('month')
    ...
    for month in range(1, 13):
        if month not in aggregated_monthly['month'].values:
            aggregated_monthly = pd.concat([aggregated_monthly, pd.DataFrame({'month':[month], 'capacity_factor':[aggregated_monthly['capacity_factor'].mean()]})])

    aggregated_monthly['name'] = unit[0]
    aggregated_monthly['unit_id'] = unit[1]

    aggregated_monthly.sort_values('month', inplace=True)
    all_units_monthly_capacity_factor = pd.concat([all_units_monthly_capacity_factor, aggregated_monthly])
...
# for dev purposes:
    if False:
        if aggregated_monthly.capacity_factor.min() < -.2:
            ...
            plt.plot(aggregated_monthly['month'], aggregated_monthly['capacity_factor'], label='Capacity Factor')
            plt.scatter(aggregated_monthly['month'], aggregated_monthly['capacity_factor'], label='Capacity Factor')
            plt.legend()
            plt.xlabel('Timestamp')
            plt.ylabel('Capacity Factor')
            plt.title(f'Capacity Factor for Generator {unit[0]}: {unit[1]}. min/max: {round(aggregated_monthly["capacity_factor"].min(),1)} / {round(aggregated_monthly["capacity_factor"].max(),1)}')
...
list_of_monthly_capacity_factor = list_of_monthly_capacity_factor - mean_capacity_factor[:, np.newaxis]
...
clean_capacity_per_month = list_of_monthly_capacity_factor2 + mean_capacity_factor[:, np.newaxis]
...
clean_capacity_df.to_csv('clean_capacity_MISO.csv', index=False)

# Melt the DataFrame to long format
clean_capacity_df = clean_capacity_df.melt(id_vars=['unit_id', 'name'], var_name='month', value_name='capacity_factor_clean')
...
for unit in all_units.values:
    unit_data = df.query('unit_id == @unit[1] and name == @unit[0]').sort_values('case_converted')
    aggregated_monthly = unit_data.groupby('month').agg({'capacity_factor':'mean', 'capacity_factor_clean':'mean'}).reset_index().sort_values('month')
    ...
    plt.plot(aggregated_monthly['month'], aggregated_monthly['capacity_factor'], label='Capacity Factor')
    plt.scatter(aggregated_monthly['month'], aggregated_monthly['capacity_factor'], label='Capacity Factor')
    plt.plot(aggregated_monthly['month'], aggregated_monthly['capacity_factor_clean'], label='Capacity Factor Clean')
    plt.legend()
    plt.xlabel('Timestamp')
    plt.ylabel('Capacity Factor')
    plt.title(f'Capacity Factor for Generator {unit[0]}: {unit[1]}. min/max: {round(aggregated_monthly["capacity_factor"].min(),1)} / {round(aggregated_monthly["capacity_factor"].max(),1)}')
...
if False:
    ...
    result = df.groupby(['name','unit_id','month']).agg({'actual_pg':'mean','pmax_Actual':'mean'}).reset_index()
    result['capacity_factor'] = result['actual_pg'] / result['pmax_Actual']

    # merge result back into df
    df = df.merge(result[['generator_uid','unit_id','month','capacity_factor']], on=['generator_uid','unit_id','month'], how='left').sort_values('timestamp')

    for generator_uid in df['generator_uid'].unique():
        generator = df[df['generator_uid'] == generator_uid]
        plt.figure(figsize=(15, 11))
        plt.plot(generator['timestamp'], generator['capacity_factor'], label='Capacity Factor')
        plt.plot(generator['timestamp'], generator['actual_pg'] / generator['pmax_Actual'], label='Forecast Capacity Factor')    
        plt.legend()
        plt.xlabel('Timestamp')
        plt.ylabel('Capacity Factor')
...
if False:
    ...
    if market_name == "miso":
        ...
        if not 'miso_plant_name_and_coordinates.csv' in os.listdir(this_dir):
            os.system("cp /Users/michael.simantov/Documents/miso_plant_name_and_coordinates.csv .")
        ...
        if not 'spp_plant_name_and_coordinates.csv' in os.listdir(this_dir):
            os.system("cp /Users/michael.simantov/Documents/spp_plant_name_and_coordinates.csv .")
        df_EIA_generators = pd.read_csv("spp_plant_name_and_coordinates.csv")

    df = df.join(df_EIA_generators.set_index('uid')[['latitude','longitude','energy_source_code','prime_mover_code']], on='generator_uid')
    ...
    def calculate_error_metrics(df):
        ...
        df['Pmax_Error'] = (df['actual_pg'] - df['pmax_Forecast']) / df['pmax_Forecast']                  # Pmax_diff
        df['Pmax_error_percentage'] = (df['Error'] / df['pmax_Actual']) * 100                             # Pmax_error_percentage
        ...
        # replace with 0
        df['Percentage_Error'] = df['Percentage_Error'].replace([np.inf, -np.inf, np.nan], 0)
    ...
    results = pd.DataFrame(results.tolist(), index=results.index, columns=['MAE', 'MPE', 'Bias', 'Corr', 'Pmax_diff', 'Pmin_diff', 'Pmax_error_percentage'])
    ...
    if False:
        for column in ['zone_uid', 'fuel_type']:
            for criterion in ['Pmax_error_percentage', 'Pmax_diff']:
                all_results = df.groupby(column).agg({criterion:['mean','std'], 'Bias':['mean','std'], 'latitude':'median', 'longitude':'median'}).reset_index()
                all_results.sort_values(column, inplace=True)
                all_results['row_count'] = df.groupby([column]).apply(count_generators).rename('temp').reset_index().sort_values(column)['temp'].values
                ...
                results = all_results.reindex(all_results[criterion]['mean'].abs().nlargest(10).index)

                # Create a figure and a set of subplots, arranged side by side
                fig, axs = plt.subplots(1, 2, figsize=(32, 12), gridspec_kw={'width_ratios': [1, 2]})

                # Plot 1: Bar chart for MPE on the first subplot
                axs[0].bar(results[column], results[criterion]['mean'], yerr=results[criterion]['std'], color='skyblue')
                ...
                size_factor = 2000  # Adjust this factor to scale the sizes appropriately

                # Use the mean of MPE to determine the size of the circle
                sizes = np.sqrt(np.abs(results[criterion] / np.abs(mape_mean))) * size_factor

                # Plot 2: Scatter plot on the second subplot
                sc = axs[1].scatter(results['longitude'], results['latitude'], s=sizes['mean'], c=sizes['std'], cmap='viridis', alpha=.5, edgecolors='w')
                axs[1].scatter(df['longitude'], df['latitude'], sizes=[.5]*len(df))
                axs[1].scatter(df[df[column].isin(results[column])]['longitude'], df[df[column].isin(results[column])]['latitude'], sizes=[15]*len(df))
                ...
                for index, row in results.iterrows():
                    axs[1].text(row['longitude']['median'], row['latitude']['median'], f"{row[column].values[0]}\n({row['row_count'].values[0]})", fontsize=10, ha='center', va='bottom', fontweight='bold')

                plt.suptitle(f'Analysis Visualization per {column}   |   Pmax_diff = (actual_pg - pmax_Forecast) / pmax_Forecast    |    Pmax_error_percentage = max[(fcst_pg - Actual_pg) / pmax_Actual]')

                plt.tight_layout(rect=[0, 0.03, 1, 0.95])  # Adjust the layout to make room for the suptitle

                plt.title(f'Scatter plot with circle size proportional to {criterion} mean and color by {criterion} std')
    ...
    if True:
        for column in [('fuel_type','zone_uid'),('zone_uid','fuel_type'),('prime_mover_code', 'energy_source_code')]:
            criterion_1 = column[0]
            criterion_2 = column[1]
            for Error_type in ['Pmax_error_percentage','Pmax_diff']:
                results = df.groupby([criterion_1,criterion_2]).agg({Error_type:['mean','std'], 'Bias':['mean','std']}).reset_index()
                results.sort_values([criterion_1, criterion_2], inplace=True)
                results['row_count'] = df.groupby([criterion_1, criterion_2]).apply(count_generators).rename('temp').reset_index().sort_values([criterion_1, criterion_2])['temp'].values
                results.sort_values([criterion_1, (Error_type, 'mean'),criterion_2], inplace=True)
                ...
                results['label'] = results[criterion_1] + ' | ' + results[criterion_2].astype(str)
                ...
                colors = ['cyan', 'red']  # Define more colors if you have more than two fuel_types
                color_map = {f: colors[i % len(colors)] for i, f in enumerate(fuel_types)}

                # Apply colors based on criterion
                bar_colors = [color_map[f] for f in results[criterion_1]]

                plt.bar(results['label'], np.sign(results[Error_type]['mean'])*np.sqrt(np.sqrt(np.abs(results[Error_type]['mean']))), yerr=np.sqrt(np.sqrt(np.abs(results[Error_type]['std']))), color=bar_colors)
                plt.xticks(rotation=45, ha="right")

                # Extract unique fuel_types to determine color change
                unique_fuel_types = results[criterion_1].unique()
                colors = ['black', 'red']  # Define more colors if you have more than two fuel_types
                # Create a color map for alternating colors
                color_map = {f: colors[i % len(colors)] for i, f in enumerate(unique_fuel_types)}

                # Apply alternating colors to x-axis labels based on fuel_type
                tick_labels = plt.gca().get_xticklabels()
                for label in tick_labels:
                    f = label.get_text().split(' | ')[0]  # Extract criterion from the label
                    label.set_color(color_map[f])

                for index, row in results.iterrows():
                    plt.text(index, np.sign(row[(Error_type, 'mean')])*np.sqrt(np.sqrt(np.abs(row[(Error_type, 'mean')]))), str(row['row_count'].values), ha='center')
                    
                plt.axhline(y=0, color='r', linestyle='--')

                plt.title(f'{Error_type} for Each {criterion_1}   |   Pmax_diff = (actual_pg - pmax_Forecast) / pmax_Forecast    |    Pmax_error_percentage = max[(fcst_pg - Actual_pg) / pmax_Actual]')
    ...
    for fuel_type in df['fuel_type'].unique():
        ...
        this = results.groupby('generator_uid').agg({'Corr':'mean', 'MPE':'mean', 'MAE':'mean', 'Bias':'mean'}).reset_index()
    ...
    for zone in df['zone_uid'].unique():
        ...
        plt.figure(figsize=(15, 11))
        plt.scatter(results['MPE'], results['MPE'], s=results['MAE'], c=results['Bias'], cmap='viridis', alpha=0.5, edgecolors='w')
        plt.colorbar(label='Bias')
        plt.xlabel('MPE Mean')
        plt.ylabel('MPE Std')
    ...
    for zone in df['zone_uid'].unique():
        results = df[df['zone_uid'] == zone]
        plt.figure(figsize=(15, 11))
        plt.scatter(results['Corr'], results['MPE'], s=results['MAE'], c=results['Bias'], cmap='viridis', alpha=0.5, edgecolors='w')
        plt.colorbar(label='Bias')
        plt.xlabel('Correlation')
        plt.ylabel('MPE')
    ...
    if False:
        ...
        high_correlation_pairs = error_correlation[(error_correlation.abs() > threshold) & (error_correlation.abs() < 1.0)].stack().index.tolist()

        if high_correlation_pairs:
            print("Generators with high error correlations:")
            for pair in high_correlation_pairs:
                print(f"{pair[0]} and {pair[1]}: {error_correlation.loc[pair[0], pair[1]]:.2f}")
        else:
