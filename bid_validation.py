...
class BidValidator:
    ...
    def load_cloud_data(self) -> bool:
        ...
        try:
            ...
            print(f"Loaded {len(self.supply_curves)} supply curves and {len(self.resource_db)} resources")
            return True
    ...
    def get_generator_actual_generation(self, generator_name: str) -> Optional[pd.DataFrame]:
        """
    ...
    def validate_first_block_vs_pmin(self, generator_name: str) -> Optional[BidValidationResult]:
        """
        ...
        try:
            ...
            if first_block_quantity < threshold:
                severity = BidValidationLevel.HIGH if first_block_quantity < pmin * 0.8 else BidValidationLevel.MEDIUM
    ...
    def validate_last_block_vs_generation(self, generator_name: str) -> Optional[BidValidationResult]:
        """
        ...
        try:
            ...
            lookback_hours = min(self.validation_thresholds['lookback_hours'], len(generation_data))
            ...
            gen_percentile_value = np.percentile(recent_generation, generation_percentile)
            ...
            if last_block_quantity < gen_percentile_value:
                # Determine severity
                ratio_to_max = last_block_quantity / max_observed_gen if max_observed_gen > 0 else 1
                if ratio_to_max < 0.7:
    ...
    def validate_pmax_vs_generation(self, generator_name: str) -> Optional[BidValidationResult]:
        """
        ...
        try:
            ...
            if pmax < gen_90th_percentile:
                # Determine severity
                ratio_pmax_to_90th = pmax / gen_90th_percentile if gen_90th_percentile > 0 else 1
    ...
    def validate_bid_curve_consistency(self, generator_name: str) -> List[BidValidationResult]:
        """
        ...
        try:
            ...
            for i in range(1, len(blocks)):
                ...
                if current_quantity <= previous_quantity:
                    results.append(BidValidationResult(
                        generator_name=generator_name,
                        plant_id=self._get_plant_id(generator_name),
                        unit_id=self._get_unit_id(generator_name),
            ...
            for i in range(1, len(blocks)):
                ...
                if previous_price > 0 and current_price > previous_price * price_jump_factor:
                    results.append(BidValidationResult(
                        generator_name=generator_name,
                        plant_id=self._get_plant_id(generator_name),
                        unit_id=self._get_unit_id(generator_name),
                        ...
                        details={
                            ...
                            'price_ratio': current_price / previous_price if previous_price > 0 else float('inf'),
    ...
    def validate_multi_unit_consistency(self, generator_name: str) -> List[BidValidationResult]:
        """
    ...
    def validate_single_generator(self, generator_name: str) -> List[BidValidationResult]:
        ...
        results.extend(multi_unit_results)  # Will be empty list since no validation is performed
    ...
    def run_comprehensive_validation(self, generator_list: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        print(f"📊 Data Status:")
        print(f"  • Supply curves available: {len(self.supply_curves) if self.supply_curves else 0}")
        print(f"  • Resource DB available: {len(self.resource_db) if self.resource_db else 0}")
        ...
        if generator_list is None:
            ...
            # Filter to only include generators with actual bid blocks and not retired
            ...
            # For specified list, only validate generators with bid blocks and not retired
            generators_to_validate = []
            retired_count = 0
            for generator_name in generator_list:
                ...
                if (generator_name in self.supply_curves and 
                    self.supply_curves[generator_name].get('offer_curve', {}).get('blocks', [])):
                    generators_to_validate.append(generator_name)
        
        print(f"Validating {len(generators_to_validate)} active generators with bid blocks...")
        print(f"  - Generators with supply curves: {len(self.supply_curves)}")
        print(f"  - Active generators with non-empty bid blocks: {len(generators_to_validate)}")
        ...
        if self.supply_curves:
            for generator_name in self.supply_curves.keys():
                if not self._is_generator_retired(generator_name):  # Only count active generators
                    supply_curve = self.supply_curves[generator_name]
                    offer_curve = supply_curve.get('offer_curve', {})
                    blocks = offer_curve.get('blocks', [])
                    if not blocks:
        ...
        if empty_blocks_count > 0:
            print(f"  - Active generators with empty bid blocks (excluded from validation): {empty_blocks_count}")
        
        # Run validation for each generator
        all_results = []
        for i, generator_name in enumerate(generators_to_validate):
            if i % 100 == 0:
                print(f"Progress: {i}/{len(generators_to_validate)} generators processed")
    ...
    def generate_summary_report(self) -> pd.DataFrame:
        ...
        summary_data = {
            ...
            'unique_generators_affected': len(set(r.generator_name for r in self.validation_results)),
            'timestamp': datetime.now().isoformat(),
    ...
    def save_results(self, filename_prefix: str = "bid_validation", generate_pdf: bool = True) -> None:
        """
        ...
        if self.validation_results:
            ...
            for result in self.validation_results:
                row = {
                    ...
                    'plant_id': result.plant_id,
                    'unit_id': result.unit_id,
                    'fuel_type': self._get_fuel_type(result.generator_name) or 'Unknown',
                    'validation_type': result.validation_type.value,
                    'severity': result.severity.value,
                    'message': result.message,
                    'timestamp': result.timestamp,
            ...
            if results_data:
                results_df = pd.DataFrame(results_data)
                detailed_filename = f"{filename_prefix}_detailed_{self.market}_{timestamp}.csv"
                ...
                if generate_pdf:
                    try:
                        ...
                        report_generator = PerformanceReportGenerator(self.config if hasattr(self, 'config') else None)
                        ...
                        for gen_name in unique_generators:
                            gen_issues = results_df[results_df['generator_name'] == gen_name]
                            
                            # Calculate bid validation performance score based on severity
                            critical_count = len(gen_issues[gen_issues['severity'] == 'critical'])
                            high_count = len(gen_issues[gen_issues['severity'] == 'high'])
                            medium_count = len(gen_issues[gen_issues['severity'] == 'medium'])
                            ...
                            rmse_value = max(1.0, (100 - score) * 0.5)  # Higher issues = higher RMSE
                        ...
                        high_severity_issues = results_df[results_df['severity'].isin(['critical', 'high'])]
                        bid_alerts = []
                        ...
                        print(f"   Report includes {len(performance_data)} generators with validation results")
                        
                    except ImportError:
                        print("⚠️  Warning: matplotlib/seaborn not available - PDF report generation skipped")
        ...
        if not summary_df.empty:
            summary_filename = f"{filename_prefix}_summary_{self.market}_{timestamp}.csv"
            ...
            if generator_name in self.resource_db:
                ...
                if generators:
                    if len(generators) == 1:
                        ...
                        # Multi-unit resource
                        unit_ids = [gen.get('eia_uid', {}).get('unit_id') for gen in generators]
...
def integrate_with_generator_analyzer(analyzer_instance, run_validation: bool = True):
    ...
    if not validation_results.empty:
        ...
        critical_issues = validation_results[validation_results['severity'] == 'critical']
        ...
        if not results.empty:
            ...
            if not critical_high.empty:
                ...
                if len(critical_high) > 10:
                    print(f"  ... and {len(critical_high) - 10} more critical/high issues")
            ...
            if not summary.empty:
                print(f"\n📊 SUMMARY STATISTICS:")
                print(f"  • Unique generators affected: {summary['unique_generators_affected'].iloc[0]}")
